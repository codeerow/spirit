...
import com.codeerow.pandora.box.mvvm.view.rx.RxDialogFragment
...
abstract class MvvmDialogFragment<VS : Any, E : Any, VM : MvvmViewModel<VS, E>> : RxDialogFragment() {
...
//        dispatch(Event.UpdateUI())
    ...
    fun <T : Any?> Observable<T>.subscribeByView(onNext: (T) -> Unit = {}): Disposable {
        return subscribe(onNext) { dispatch(this@MvvmDialogFragment, Event.Exception(it)) }
                .bindToLifecycle()
    }
    ...
    fun Completable.subscribeByView(onComplete: () -> Unit = {}): Disposable {
        return subscribe(onComplete) { dispatch(this@MvvmDialogFragment, Event.Exception(it)) }
                .bindToLifecycle()
    }
    ...
    fun dispatch(fragment: Fragment, event: Event.Segue) {
        (fragment as? NavFragment)?.handle(event)?.also { return }
